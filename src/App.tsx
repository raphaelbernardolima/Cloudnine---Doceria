...
export function App() {
  ...
  useEffect(() => {
    const handleCartUpdate = (payload: any) => {
      console.log('Event Triggered: CART_UPDATED', payload);
      if (payload.added?.product?.nome) {
        showToast(`${payload.added.product.nome} adicionado ao carrinho!`);
      }
    };
    globalEventBus.on(AppEvents.CART_UPDATED, handleCartUpdate);
    return () => {
      globalEventBus.off(AppEvents.CART_UPDATED, handleCartUpdate);
    };
  }, []);
